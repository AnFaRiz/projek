...
url24<-"http://dosen.unand.ac.id/web/pencarian?cari=X&act=dir&_tog1149016d=all"
x1 <- read_html(url24) %>% html_nodes("table") %>% .[[1]] %>% html_table()
x2 <- read_html(url24) %>% html_nodes("table") %>% .[[1]] %>% html_table()
x11 <- t(x1[,2])
colnames(x11)<-as.vector(x1[,1])$X1
rownames(x11)<-NULL
x22 <- x2
